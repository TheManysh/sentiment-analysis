...
from predict.predictApp import PredictApi
...
def predictRoute():
    try:
        if request.json['text'] is not None and request.json['userId'] is not None and request.json['projectId'] is not None:
            ...
            jsonFilePath = trainingDataFolderPath + userId + \
                "/" + projectId + "/trainingData.json"
            modelPath = trainingDataFolderPath + userId + \
                "/" + projectId + "/modelForPrediction.sav"
            vectorPath = trainingDataFolderPath + userId + \
                "/" + projectId + "/vectorizer.pickle"
            result = clntApp.predictObj.executePreocessing(
                text, jsonFilePath, modelPath, vectorPath)
...
def deleteUserProjectFolder():
    try:
        if request.args.get("userId") is not None:
            userIdAndProjectId = trainingDataFolderPath + \
                request.args.get("userId")
        if request.args.get("projectId") is not None:
            userIdAndProjectId = userIdAndProjectId + \
